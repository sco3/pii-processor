...
fn user_content(text_set: &mut HashSet<String>, chat_log: &mut String, content: &String) {
    ...
    if let Some(start_idx) = content.find(USER_TAG_START) {
        if let Some(end_idx) = content.find(USER_TAG_END) {
            ...
            add_txt(text_set, chat_log, &format!("{}: {}\n", USER, user_input));
        } else {
...
fn assistant_content(text_set: &mut HashSet<String>, chat_log: &mut String, content: &String) {
...
pub fn add_txt(history: &mut HashSet<String>, dst: &mut String, src: &str) {
